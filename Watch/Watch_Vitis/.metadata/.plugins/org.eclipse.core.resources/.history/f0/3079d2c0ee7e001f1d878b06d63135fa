...
int main() {
    ...
    while (1) {
    	...
    	scanf("%d", &data);
    	Xil_Out32((XPAR_LAB3_MATBI_0_BASEADDR) + (0*AXI4_DATA), (u32)(data));
    	printf("WATCH (hour : min : sec)\n");
		while(1) {
    		data = Xil_In32((XPAR_LAB3_MATBI_0_BASEADDR) + (1*AXI4_DATA));
			sec = data & 0x3F;
			min = (data>>6) & 0x3F;
			hour = (data>>12) & 0x1F;
    		printf("\r(%2d:%2d:%2d)", hour, min, sec);
			fflush(stdout); // I can see output data of remaining in the buffer immediately.
